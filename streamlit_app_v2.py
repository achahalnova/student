...
if page == "Results Analyzer (Excel)":
    ...
    with col1:
        dat_file = st.file_uploader("Upload Student Data File (.dat/.iit)", type=["dat", "iit", "txt"])
    with col2:
    ...
    if dat_file is not None and answer_key_file is not None:
        ...
        if df_answer_key is not None:
            if st.button("Grade DAT File"):
                if "jee" not in selected_batch.lower():
                    st.warning("Note: The grading logic for NEET batches has not been defined yet! The current processing rules only apply to JEE.")
                else:
                    ...
                    try:
                        ...
                        st.download_button(
                            ...
                            data=out_buffer,
                            file_name="Graded_Student_Results.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                        )
                    except Exception as e:
                        st.error(f"Error grading DAT file: {e}")
